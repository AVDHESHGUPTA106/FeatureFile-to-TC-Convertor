...
public class FeatureClassificationController {
	...
	public Object authenticate(@Valid @ModelAttribute(name = "remoteRequest") RemoteRequest remoteRequest,
			BindingResult result, Model model) {
		try {
			if (ValidateService.isValidGITRepo(remoteRequest.getUrl()) == true) {
				...
				} else if (responseFrmGitUrl instanceof ResponseEntity) {
					model.addAttribute("remoteRequest", remoteRequest);
					return generatePRExcelSheet(remoteRequest, responseFrmGitUrl, result);
				}
			} else {
				ObjectError error = new ObjectError("globalError", "Error: Invalid Git Repository");
				result.addError(error);
			}
		} catch (Exception e) {
	 ...
	 */
	public Object generatePRExcelSheet(RemoteRequest remoteRequest, Object responseFrmGitUrl, BindingResult result)
			throws URISyntaxException {
		List<PRDetail> prDetails = getPRDetails(remoteRequest, responseFrmGitUrl);
		if (prDetails.size() == 0) {
			ObjectError error = new ObjectError("globalError",
					"Info: PR comment(s) are not found for selected date range");
			result.addError(error);
			return "index";
		}
	...
	private List<PRDetail> getPRDetails(RemoteRequest remoteRequest, Object responseFrmGitUrl)
			throws URISyntaxException {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			if(StringUtils.equalsIgnoreCase("open", obj.getString("state"))) {closedDate = ZonedDateTime.now();}
			...
			Comparator<PRComment> comparator = (c1, c2) -> { 
				return ZonedDateTime.parse(c1.getCreationDate(), formatter.withZone(ZoneId.of("UTC"))).toLocalDateTime()
						.compareTo(ZonedDateTime.parse(c2.getCreationDate(), formatter.withZone(ZoneId.of("UTC"))).toLocalDateTime());
			};
			...
			PRDetailBuilder prDetailBuilder = PRDetail.builder()
					...
					.repoName(repoName)
					.prDescription(obj.getString("body"))
					...
					.prAgeing(prAgeing)
					.prCommentList(prCombinedCommentList)
					.isParent(true);
	...
	private List<PRComment> getPRComments(String uri, String accessToken) throws URISyntaxException {
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			JSONObject obj = (JSONObject) (jsonArray.getJSONObject(i));
			prCommentList.add(PRComment.builder()
					.prComment(obj.getString("body"))
					.userName(obj.getJSONObject("user").getString("login"))
					.creationDate(obj.getString("updated_at"))
					.isRvwComment(true)
					.build());
		}
	...
	private List<PRComment> getPRTimelineComments(String uri, String accessToken, String title)
		...
		for (int i = 0; i < jsonArray.length(); i++) {
			...
			if(StringUtils.isEmpty(createdAt) && obj.has("event") && "review_requested".equalsIgnoreCase(obj.getString("event"))) {
			...
			if(null != closedDate && StringUtils.isEmpty(prAgeing) && StringUtils.isNotEmpty(createdAt)) {
				Period between = Period.between(ZonedDateTime.parse(createdAt, formatter.withZone(ZoneId.of("UTC"))).toLocalDate(), closedDate.toLocalDate());
